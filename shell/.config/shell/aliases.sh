...
then
  ...
  alias gsu1='git branch --set-upstream-to=origin/"$(git symbolic-ref --short HEAD)" "$(git symbolic-ref --short HEAD)"'
  alias gpush='git push'
  alias gf='git fetch'
  alias gpushu1='git push --set-upstream $(git remote show) $(git rev-parse --abbrev-ref HEAD)'
